...
/**
 * @note: gflags definitions 这是一套命令行参数解析工具
 * DEFINE_bool, DEFINE_string等宏定义用于定义命令行参数
 * gflags主要支持的参数类型有bool、int、double、string等
 * 参数定义通过DEFINE_type完成，宏的第一个参数是参数名，第二个参数是默认值，第三个参数是参数描述
 * 当参数定义后，可以通过FLAGS_parameter_name来访问参数值
 */
...
int main(int argc, char** argv) {
  ...
  // check：为glog日志库中的提供的系列宏，检测某值表达式是否为真
  // 检测expression是否为true，如果不为true则输出错误信息并终止程序
