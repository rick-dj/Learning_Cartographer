...
class Node {
 ...
 private:
  ...
  // GUARDED_BY: 是数据成员的属性，该属性声明数据成员给定功能保护
  // map_builder_bridge_: 是一个智能指针，指向MapBuilderBridge对象 受到给定的mutex_保护
  ...
  // map  的存储是无序的
  // unordered_map 的存储是有序的，这个采用哈希数据结构实现的，内部数据保存的效率是最高的
  // :: 全局命名空间作用域运算符
